...
function Twitter(twitterDevice) {
  if (!(this instanceof Twitter)) return new Twitter(twitterDevice);

  var defaults = {};

  this.device  = twitterDevice;
  this.options = utils.merge(defaults, keys.urls);
}
...
Twitter.prototype.get = function(url, params, callback) {
  ...
  this.device.oauth.get(url + '?' + querystring.stringify(params),
...
Twitter.prototype.login = function(oauth_callback) {
  ...
  return function handle(req, res, next) {
    self.device.oauth.getOAuthRequestToken(
...
Twitter.prototype.callback = function() {
  ...
  return function handle(req, res, next) {
    self.device.oauth.getOAuthAccessToken(
...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  var request = this.device.oauth.post(
    url,
    this.device.oauth_access_token,
    this.device.oauth_access_token_secret,
    params, null
